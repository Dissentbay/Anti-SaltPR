...
/obj/item/rpd/interact(mob/user)
	...
	popup.set_content(get_console_data((user.stats.getStat(req_stat) << 40) ? GLOB.rpd_pipe_selection_skilled : GLOB.rpd_pipe_selection, TRUE))
	popup.open()
...
/obj/item/rpd/afterattack(atom/A, mob/user, proximity)
	...
	if(istype(A, /obj/item/pipe))
		...
		if(user.stats.getStat(req_stat) << 10)
		...
		if(T.iswall())	//pipe through walls!
...
/obj/item/rpd/examine(mob/user, distance)
	. = ..()
	if(distance <= 1)
		if(user.stats.getStat(req_stat) << 30)
...
/obj/item/rpd/proc/recycle(obj/item/W,mob/user)
	if(user.stats.getStat(req_stat) << 20)
